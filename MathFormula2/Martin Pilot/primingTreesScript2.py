...
for i,stim in enumerate(df['target']):
	...
	target = expyriment.stimuli.TextLine(stim.decode('utf-8').lstrip().rstrip(),text_size=28, text_font='Inconsolata.ttf')
	prime = expyriment.stimuli.TextLine(df["prime"][i].decode('utf-8').lstrip().rstrip(),text_size=28, text_font='Inconsolata.ttf')
...
for block in exp.blocks:
	...
	#interval between target and prime
	(response,response_time) = check_and_wait(114,exp.clock,target_prime_interval,response,response_time)
	
	#present prime
	trial.stimuli[1].present()
	prime_present_time = exp.clock.stopwatch_time
	(response,response_time) = check_and_wait(114,exp.clock,prime_time,response,response_time)
	exp.screen.clear()
	exp.screen.update()
	...
	#save data
	exp.data.add([target_present_time,df["target"][stimIndex],prime_present_time,df["prime"][stimIndex],response,response_time])
	exp.keyboard.clear()
