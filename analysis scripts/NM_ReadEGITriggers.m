...
function  [head, EventData] = NM_ReadEGITriggers(filename, dataChunks,forceversion)
...
if (segmented),
    ...
    if isempty(dataChunks)
        ...
        disp('Reading data...');
        readtotal = 0;
        read_samp = ceil(head.samples/100);
        for i = 1:99
            disp([num2str(i) '%...']);
            [TrialData, read_num] = fread(fid, [FrameVals,read_samp],datatype);
            readtotal = readtotal+read_num;
            
            % Only keep the EventData
            EventData(:,1+(i-1)*read_samp:i*read_samp) = TrialData(head.nchan+1:end,:);
        end
        
        % Add the last bit
        [TrialData, read_num] = fread(fid, [FrameVals,nframes-99*read_samp],datatype);   
        EventData(:,1+99*read_samp:end) = TrialData(head.nchan+1:end,:);
        readtotal = readtotal+read_num;
