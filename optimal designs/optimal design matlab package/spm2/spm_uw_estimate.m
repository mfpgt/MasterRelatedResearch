...
% 
%_______________________________________________________________________
% @(#)spm_uw_estimate.m	1.4 Jesper Andersson 03/03/06
...
for iter=1:ds.noi
   ...
   if iter > 1 & yty > SSx(iter-1)
      ...
      % Solve for beta
      Aty = Aty + AtA*beta;
      AtA = AtA + ds.lambda * kron(eye(nof),diag(H)) * SSx(iter)/(nscan*sum(msk));
      ...
      tmp = dispP.mat;
      dispP.mat = P(1).mat;
      spm_uw_show('FinIter',SSx,def,ds.fot,ds.sot,dispP,ds.q);
      dispP.mat = tmp;
   end
   clear AtA
end
