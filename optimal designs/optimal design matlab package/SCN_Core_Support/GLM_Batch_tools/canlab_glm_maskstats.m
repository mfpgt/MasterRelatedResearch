...
if ALLOPS, OP = {'cosine_similarity' 'dot_product' 'correlation' 'mean' 'centered_dot_product'}; end
...
elseif iscellstr(DIRS)
    ...
    allconnames = cellstr(char(SPM.xCon.name));
...
if DO_PLOTS
    ...
    for m = 1:numel(MASK)
        [ignore maskname] = fileparts(MASK{m});
...
if DO_GRPLEV
    ...
    if DO_PLOTS
        ...
        for m = 1:numel(MASK)            
            ...
            X = [];
            for b = 1:nhcons
                X = [X MASKSTATS.mask(m).grp.beta(b).mean]; %#ok
            end
    
            figure; bar(X,'group');
            title(sprintf('group level betas (mean within %s)',maskname),'interpreter','none');
            ylabel('beta');
