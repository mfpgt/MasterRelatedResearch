...
%   modeldir    = pwd
%   grpmodeldir = pwd/one_sample_t_test
    ...
    if isstruct(varargin{i})
        ...
        if isfield(DSGN,'subjects')
            for s = 1:numel(DSGN.subjects)
                ...
                sublevs{s} = fullfile(modeldir,subname{s}); %#ok
            end
        end
...
if exist('covfile','var')
    if ~exist(covfile,'file')
        ...
        system(['cut -d, -f2- < ' covfile ' > ' tempcovsfile]);
        s = importdata(tempcovsfile);
        EXPT.cov = s.data;
...
if ~OPTS.run_robfit
    ...
    if isempty(includedcons)
        ...
        if ~isempty(OPTS.parallel_dream) && numel(includedcons)>1            
            % define scheduler
...
%             if any(grfstatuses==-1), grfstatus = -1; else grfstatus = 1; end
    ...
    if OPTS.run_grf
        ...
        graymattermask = which('scalped_avg152T1_graymatter_smoothed.img');
        try
    ...
    if OPTS.run_links
        ...
        try
            ...
            if ismac
                [dummy absgrpmodeldir] = system(sprintf('cd %s; pwd -P',grpmodeldir));
            else
                [dummy absgrpmodeldir] = system(sprintf('readlink -f %s',grpmodeldir));
            end
