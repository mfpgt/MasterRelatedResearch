...
[ignore subnum] = fileparts(DSGN.subjects{s});
...
if OPTS.onlycons
    ...
    if run_this_model
        ...
        try
            diary(diaryfile)
            [runs runs3d names onsets durations pmods conditions_by_run OPTS] = prep_for_canlab_spm_fmri_model_job(DSGN,OPTS,runs,runs3d,names,onsets,durations,pmods,z);
            diary off
        catch exc
            if OPTS.nocatch, cd(STARTINGDIR); rethrow(exc)
            else diary(diaryfile), fprintf('> %s\n',getReport(exc,'extended')); diary off; end
        ...
        diary(diaryfile), fprintf('%sRUNNING model\n\t%s\n',modeljobcmd,z), diary off
        try
            eval(modeljobcmd);
...
for session = 1:numel(runs)
    ...
    for i = 1:numel(DSGN.conditions{sess})
        ...
        if ~isempty(regexp(DSGN.conditions{sess}{i},'[[]{}*?]','once'))
            matfiles = filenames(DSGN.conditions{sess}{i});
            if isempty(matfiles)
                if DSGN.allowmissingcondfiles
                    fprintf('%sWARNING: No conditions files found with regexp: %s',z,DSGN.conditions{sess}{i})
                    continue
                else
                    error('%sNo conditions files found with regexp: %s\(See DSGN.allowmissingcondfiles)',z,DSGN.conditions{sess}{i})
                end
            end
        else
            ...
            if ~exist(matfiles{1},'file')
                if DSGN.allowmissingcondfiles
                    fprintf('%sWARNING: No such conditions file: %s',z,matfiles{1})
                    continue
                else
                    error('%sNo such conditions file: %s\(See DSGN.allowmissingcondfiles)',z,matfiles{1})
                end
            end
        end
        ...
        for m = 1:numel(matfiles)
            ...
            for j = 1:numel(condinfo.name)
                if ~numel(condinfo.onset{j})
                    ...
                    % load pmods
                    try
                        pmods{session}{c} = condinfo.pmod{j}; %#ok
                    catch %#ok
                        pmods{session}{c} = emptypmod; %#ok
                    end
                end
    ...
    if isfield(DSGN,'pmods')
        for i = 1:numel(DSGN.pmods{sess})
            if ~isempty(DSGN.pmods{sess}{i})
                for j = 1:numel(DSGN.pmods{sess}{i})
                    matfile = fullfile(mfdir,DSGN.pmods{sess}{i}{j});
                    matfile = [regexprep(matfile,'.mat$','') '.mat'];
%                     if ~exist(matfile,'file'), error('> No such pmods file: %s',matfile); end
                    pmodinfo = load(matfile);                    
                    
                    for k = 1:numel(pmodinfo.name)
                        l = size(pmods{session}{i},2) + 1;
                        % initialize
                        pmods{session}{i}(l) = newpmod; %#ok                        
                        pmods{session}{i}(l).name = pmodinfo.name{k}; %#ok
                        pmods{session}{i}(l).param = pmodinfo.param{k}; %#ok
                        pmods{session}{i}(l).poly = pmodinfo.poly{k}; %#ok
                    end
                end
            end
        end
    ...
    for c = 1:numel(DSGN.concatenation)
        ...
        for r = 1:numel(DSGN.concatenation{c})
            if ~any(DSGN.concatenation{c}(r) == emptyruns)
                cat{i} = [cat{i} DSGN.concatenation{c}(r)]; %#ok                
            end
        end
        if (numel(cat{i}) > 1), i=i+1; end            
    end
else
    cat = DSGN.concatenation;
end
...
for sess = 1:numel(cat)  
    oldsess1 = cat{sess}(1);
    
    % concatenate functional data
    runs3d{sess} = []; %#ok
    for r = 1:numel(cat{sess})
        oldsess = cat{sess}(r);
...
for s = 1:numel(oldonsets)
    ...
    for c = 1:numel(oldonsets{s})
        ...
        try 
            thiscond = DSGN.singletrials{s}{c};
...
for session = 1:numel(names)
    ...
    conditions_by_run(session) = i; %#ok
end
