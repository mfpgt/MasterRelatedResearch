...
if ischar(dsgnarg)
    ...
    elseif strcmp(dsgnarg,'dsgninfo')
        ...
        return;
    elseif regexp(dsgnarg,'\.mat$') && exist(dsgnarg,'file')
        load(dsgnarg);
    else
...
elseif sum(modelstatus==-1) || sum(constatus==-1)
...
fprintf('----------------------\n')
nfail = failure_report(DSGN,modelstatus,constatus,linkstatus,histstatus,reviewstatus);
diary off
...
%%
function [nfail] = failure_report(DSGN,modelstatus,constatus,linkstatus,histstatus,reviewstatus)
