...
GA.freqConditions = [.2 .2 .2 .2];
	% vector of frequencies of each trial type; should sum to 1 for no jitter
    % if the sum is < 1, then zeros will be inserted (rest intervals)
...
GA.dflag = 0;
    % estimate d-optimality (set to 1) or a-optimality (default, set to 0)
    % d-optimality is recommended for HRF shape estimation and for
    % cases in which one wants to compare multiple effects or trial types
    % without specific planned contrasts of strong prior interest
