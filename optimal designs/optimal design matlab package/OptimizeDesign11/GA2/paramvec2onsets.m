function [evtonsets,X,paramvec,conditions] = paramvec2onsets(mspec,conditions,paramvec,varargin)
%[evtonsets,X,paramvec] = paramvec2onsets(mspec,conditions,paramvec)
%
...
for i = 1:length(conditions)
    ...
    disp('Output is stored in eventonsets variable (1st output variable returned by this function)')
for i = 1:length(evtonsets)
    %eventonsets{i} = find(X(:,i)) - 1;
    %eventonsets{i} = eventonsets{i} .* mspec.TR;
    
    disp(['Event ' num2str(i)])
    disp(num2str(evtonsets{i}'))
