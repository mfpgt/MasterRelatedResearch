...
Inp.filename = (['PrimeTrees_' date '.mat']); % <-- filename for output

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%**** Experimental conditions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Inp.nSTYPE = 8; % <--  number of stimulus types (excluding the control)
Inp.ISI = 4.0; % <--time interval between event onsets
Inp.TR = 1.5; % <--time interval between scans
Inp.dT = 0.5; % <--the greatest value dividing both the ISI and TR
Inp.nEvents = 360; % <--number of events in the design
...
Inp.CZ = zeros(Inp.nSTYPE);%eye(Inp.nSTYPE); % <-- linear combinations for theta
Inp.CZ(1,:) = [0.25 0.25 0.25 0.25 -0.25 -0.25 -0.25 -0.25];
Inp.CZ(2,:) = [0.25 0.25 -0.25 -0.25 0.25 0.25 -0.25 -0.25];
Inp.CZ(3,:) = [0.25 -0.25 0.25 -0.25 0.25 -0.25 0.25 -0.25];
% 5. max(Fe) and Max(Fd)

Inp.MaxFe = 1; %<-- numerical approximation of Max(Fe)
Inp.MaxFd = 1; %<-- numerical approximation of Max(Fe)
...
Inp.stimFREQ = [48 48 32 32 48 48 32 32]./320;
%[16 8 8 8 16 8 8 8 16 16 16 16 16 16 8 8 8 16 8 8 8 16 16 16 16 16]./320; 
% ones(1,Inp.nSTYPE)./Inp.nSTYPE; %(DEFAULT INPUT)
%<-- desired stimulus frequency
...
%Stopping rule for accumulated improvement
%Inp.StopRule = 2;
%Inp.numITR = 100; %<-- total number of generations
%Inp.improve = 10^(-7); %<-- size of each generation
